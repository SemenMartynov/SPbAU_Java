...
public class FilesystemWalker {
	...
	/**
	 * Constructor with one argument - filter. Default FilesystemPrinter will be
	 * used for display a tree.
	 * 
	 * @param filter
	...
	public void start(String startPoint) throws IllegalArgumentException {
		...
		try {
			...
			if (filter.accept(node)) {
				if (node.isDirectory()) {
					printer.printDirectory(node.getName(), 0,
							isAccessible(node));
					start(node, 1);
				} else {
	...
	/**
	 * Recursive bypass of directory tree. We get a filtered list of sub
	 * elements and iterate through it. If we find a directory - go deeper, if a
	 * file - just print it.
	 * 
	 * @param node
	 *            Current directory, which can contain sub directories and
	 *            files.
	 * @param deep
	...
	private void start(File node, int deep) {
		try {
			...
			// If we can't get access to folder - we can't get access to sub
			// folders
			if (nodes == null) {
				return;
			}
			// Sort directories before files, and then lexicographical sorting
			// by names
			...
			for (File subNode : nodes) {
				...
				if (subNode.isDirectory()) {
					printer.printDirectory(subNode.getName(), deep,
							isAccessible(subNode));
					// if last element is a directory - we need a special steep
					// for nice tree
					...
					// For file - just print
					printer.print(subNode.getName(), deep,
							isAccessible(subNode));
				}
			}
	 ...
	 * 
	 * @param node
	 *            the file, which is need to be checked for access.
